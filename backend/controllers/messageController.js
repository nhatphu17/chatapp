...
const messageController = {
  store: async (req, res, io) => {
    ...
    try {
      ...
      let msgStatus = "read";
      await prisma.$executeRaw(
        Prisma.sql`INSERT INTO Message (content,imageUrl, chatId, userId, status)
                    VALUES (${content},${imageUrl}, ${chat.id}, ${userId}, ${msgStatus});`
      );

      io.emit("newMessage", { content,imageUrl, chatId: chat.id, userId });

      res
        .status(201)
        .json({ message: { content,imageUrl, chatId: chat.id, userId }, chat });
    } catch (error) {
